<template>
  <v-app>
    ...
    >
      Copied!
      <v-btn dark text @click="snackbar_copy_success = false">
        Close
      </v-btn>
    </v-snackbar>
    ...
    >
      ...
      <v-btn dark text @click="snackbar_copy_error = false">
        Close
      </v-btn>
    </v-snackbar>

    <v-navigation-drawer app disable-route-watcher v-model="drawer" temporary>
      <template v-slot:prepend>
        <v-list-item selectable>
          <v-list-item-content>
            <v-list-item-title>{{ $store.state.board.name }}</v-list-item-title>
            <v-list-item-subtitle
              >Code: {{ $store.state.board.code }}
              <v-btn text x-small @click="copy_code">Copy</v-btn>
            </v-list-item-subtitle>
          </v-list-item-content>
        </v-list-item>
      </template>
      ...
      <v-list nav dense>
        <v-list-item link @click="copy_link">
          <v-list-item-icon>
            <v-icon>share</v-icon>
          </v-list-item-icon>
          <v-list-item-content>
      ...
      <template v-slot:append>
        <v-list nav>
          <v-list-item>
            <v-list-item-icon>
              <v-icon color="success darken-2" v-if="!$store.state.syncing"
                >done</v-icon
              >
              <v-progress-circular
                indeterminate
        ...
        <div class="pa-2">
          <v-btn block color="error lighten-1" @click="logout">Logout</v-btn>
        </div>
      </template>
    </v-navigation-drawer>
